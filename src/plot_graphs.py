...
def create_graph(df, edges_norm=False, lower_bound_edges=None):
    net = Network()
    ...
    if lower_bound_edges is not None:
        ...
        list_index = series_count_src[series_count_src > lower_bound_edges].index
            
    if edges_norm:
        df['edge_norm'] =  get_softmax_norm(df['edge'])
        
    print('lenght src nodes', len(list_index), lower_bound_edges)
    print(df.shape)
